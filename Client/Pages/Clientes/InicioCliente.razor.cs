...
namespace VentaAutos.Client.Pages.Clientes
{
    public partial class InicioCliente
    {
        ...
        public string ENDPOINT_NAME { get; set; } = "clientes";
        public string DTO { get; set; } = "/paginacion";//Si tiene ruta get por DTO, colocar /DTO/paginacion, sino /paginacion

        public List<cliente> lst_datos { get; set; }
        private cliente modelo = new cliente();
        ...
        private async Task Cargar ()
        {
            ...
            var httpResponse = await repositorio.Get<List<cliente>>(ruta);
        ...
        private async Task Crear ()
        {
            ...
            if (modelo.id_cliente != 0)//Es Update
            {
                //modelo.car_codusr_modificacion = ClaimUser.CodigoUsuario(await authentication);
                var httpResponse = await repositorio.Put($"api/{ENDPOINT_NAME}/{modelo.id_cliente}", modelo);
                if (httpResponse.Error)
                {
        ...
        private async Task Editar (int codigo, bool borrar)
        {
            ...
            var httpResponse = await repositorio.Get<cliente>($"api/{ENDPOINT_NAME}/{codigo}");
            if (httpResponse.Error)
            {
        ...
        private async Task Borrar (cliente modelo)
        {
            var responseHttp = await repositorio.Delete($"api/{ENDPOINT_NAME}/{modelo.id_cliente}");
